...
* A partir d'ici, à chaque fois que le client doit accéder à une API du serveur, voici ce qui va se passer :
    ...
    * Le client ajoute le jeton dans le header HTTP `Authorization` de sa requête.
...
Voici le commit correspondant :

![](images/image_000002.png)
...
Le fichier `securedhttp.service.ts` définit un service `SecuredHttp`. L'objectif est que ce service puisse remplacer de façon totalement transparente le service `Http` que nous avons utilisé jusqu'ici.

Ainsi, par exemple, nous pourrons simplement modifier la classe `MemberService` pour qu'elle utilise notre service HTTP sécurisé au lieu du service HTTP standard. A part un petit changement dans la déclaration du service dans le constructeur (et la correction de l'un ou l'autre petit bug qui trainaient dans la V07), il n'y a aucun impact sur l'utilisation du service :

![](images/image_000010.png)

Voyons maintenant comment fonctionne ce service HTTP sécurisé.

Du côté angular, nous utilisons la librairie `angular2-jwt` qui offre une couche au dessus de JWT, spécialisée pour angular. Elle a pour principal intérêt de nous offrir un service `AuthHttp` qui va permettre de faire des requêtes HTTP en y insérant automatiquement l'en-tête de sécurité contenant le token.

Pour que `AuthHttp` sache où il doit récupérer le token quand il en a besoin, il faut ajouter le code suivant dans `app.module.ts` :
...
Dans la fonction `authHttpServiceFactory`, on crée l'instance de `AuthHttp` en lui passant comme paramètre un objet de type `AuthConfig` dans lequel on indique comment `AuthHttp` peut retrouver le token en associant une fonction lambda au paramètre `tokenGetter`. Dans ce cas-ci, dans cette lambda, on va chercher le token dans le *sessionStorage* avec la clé d'acces `id_token`. Nous verrons plus loin que c'est bien là que nous l'avons préalablement stockée au moment du login.

Ensuite, dans la partie `providers` du module, on précise que angular doit utiliser notre fonction `authHttpServiceFactory` comme *factory* (cf. pattern *factory*) quand il doit créer une instance du service `AuthHttp` (pour rappel, l'instanciation des services et la gestion du cycle de vie de ces instances est la responsabilité d'angular).
